...
parquet_file = profile_dir / f"{profiles.scenario_name.value}_{profiles.target_year}.parquet"
...
for tcol in time_cols:
    ...
    if len(net_case.line) > 0:
        ...
        if "length_km" in net_case.line.columns:
            lengths_km = net_case.line["length_km"].fillna(1.0)
        else:
            lengths_km = pd.Series(1.0, index=net_case.line.index)

        from_bus_v_kv = net_case.line["from_bus"].map(net_case.bus["vn_kv"]).fillna(0.0)
        delta_line_kw = np.sqrt(3.0) * from_bus_v_kv * delta_i_ka * 1000.0
    ...
    if len(net_case.trafo) > 0:
        ...
        delta_trafo_kw = delta_sn_mva * 1000.0
