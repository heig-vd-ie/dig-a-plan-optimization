...
class LongTermScenarios(BaseModel):
    n_lt_scenarios: int = Field(
        default=100, description="Number of long-term scenarios"
    )
    n_st_scenarios: int = Field(
...
class SDDPConfig(BaseModel):
    ...
    n_second_stage: int = Field(
        default=10, description="Number of second-level optimization per stage"
    )
