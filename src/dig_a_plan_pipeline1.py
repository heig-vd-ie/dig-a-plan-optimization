...
class DigAPlan():
    ...
    def solve_models_pipeline(self, max_iters: int, bender_cut_factor: float = 1.0) -> None:
        for k in range(max_iters):
            print(f"\n--- BENDERS ITERATION {k} ---")

            # 1) Solve master
            self.master_solver.solve(self.master_model_instance, tee=self.verbose)
            print("  # of cuts in master so far:", len(self.master_model_instance.benders_cuts))
            print("  Master objective (losses + Θ):", self.master_model_instance.objective())
            print("  Theta =", pyo.value(self.master_model_instance.Theta))

            # 2) Push d to slave and solve
            ds = {idx: int(val > 0.5) for idx, val in self.master_model_instance.d.extract_values().items()}
            self.slave_model_instance.master_d.store_values(ds)
            self.slave_solver.solve(self.slave_model_instance, tee=self.verbose)

            # 3) Check the slave’s infeasibility “w”
            w = pyo.value(self.slave_model_instance.objective)
            print("  Slave infeasibility w (total slack):", w)

            # 4) If zero slack, we’re done
            if abs(w) < self.slack_threshold:
                print(f"→ Converged after {k+1} iterations (no more infeasibilities).")
                break

            # 5) Otherwise, extract duals and add another cut
    ...
    def add_benders_cut(self, nb_iter: int, bender_cut_factor: float = 1.0) -> None:
        """
        Generate and add one Benders feasibility cut of the form:
            Theta >= w + sum_{(l,i,j)} pi_{l,i,j} * (d[l,i,j] - d_k[l,i,j])
        where
        - w is the slave’s infeasibility (sum of slacks) from the last solve,
        - pi_{l,i,j} is the dual of the corresponding slave constraint,
        - d_k is the value of the arc‐binary at the last master solve.
        """

        # 1) Slave infeasibility constant
        w = pyo.value(self.slave_model_instance.objective)

        # 2) Extract and aggregate duals per arc (l,i,j)
        duals = {}
        for cname, dv in self.slave_model_instance.dual.items():  # type: ignore
            name = cname.name
            # only care about our power/flow/voltage constraints
            if not any(name.startswith(pref) for pref in (
                "node_active_power_balance",
                "node_reactive_power_balance",
                "active_power_flow",
                "reactive_power_flow",
                ...
                continue
            # parse out the index tuple at end: "...[(l,i,j)]"
            idx = name.split("[")[-1].rstrip("]")
            l, i, j = map(int, idx.split(","))
            duals.setdefault((l,i,j), 0.0)
            duals[(l,i,j)] += dv

        # 3) Grab last-iteration arc-values d_k
        old_d = dict(self.master_model_instance.d.extract_values())

        # 4) Build the sum_term over all arcs where |pi|>0
        sum_term = 0
        for (l,i,j), pi in duals.items():
            if abs(pi) < 1e-8:
                continue
            sum_term += pi * (self.master_model_instance.d[l,i,j] - old_d[(l,i,j)])

        # 5) Form and add the cut on the single Theta variable
        cut_expr = self.master_model_instance.Theta >= w + sum_term
        self.master_model_instance.benders_cuts.add(cut_expr)
