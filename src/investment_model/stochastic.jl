...
function subproblem_builder(
    ...
    @variable(
        ...
        budget_remaining >= 0,
        SDDP.State,
        initial_value = params.initial_budget
    )
    @variable(
        subproblem,
    ...
    @variable(
        subproblem,
        expansion_committed[edge in grid.edges] >= 0,
        SDDP.State,
        initial_value = 0.0
    )
    ...
    @variable(
        subproblem,
        total_unmet_load[node in grid.nodes] >= 0,
        SDDP.State,
        initial_value = 0.0
    )
    @variable(
        subproblem,
        total_unmet_pv[node in grid.nodes] >= 0,
        SDDP.State,
        initial_value = 0.0
    )
    ...
    @constraint(
        subproblem,
        [edge in grid.edges],
        expansion_committed[edge].out == δ_capacity[edge]
    )
    ...
    @constraint(
        subproblem,
        budget_remaining.out == budget_remaining.in - investment_cost + δ_budget
    )
    ...
    if stage == 1
        ...
        @constraint(
            subproblem,
            [node in grid.nodes],
            actual_load[node].out == actual_load[node].in
        )
        @constraint(
            subproblem,
            [node in grid.nodes],
            actual_pv[node].out == actual_pv[node].in
        )
        ...
        @constraint(
            subproblem,
            [node in grid.nodes],
            total_unmet_pv[node].out == total_unmet_pv[node].in
        )
        @constraint(
            subproblem,
            objective_value ==
            discount_factor * (
                ...
                sum(
                    params.penalty_costs_pv[node] * total_unmet_pv[node].out for
                    node in grid.nodes
                )
            )
        )
        ...
        for node in grid.nodes
            if node == grid.external_grid
                ...
                @constraint(
                    subproblem,
                    sum(
                        flow[(edge.id, edge.from, node)] for
                        edge in grid.edges if edge.to == node
                    ) - sum(
                        flow[(edge.id, node, edge.to)] for
