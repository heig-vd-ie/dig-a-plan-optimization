...
def slave_model_parameters(model: pyo.AbstractModel) -> pyo.AbstractModel:
    ...
    model.i_max = pyo.Param(model.L, initialize=1.0e4) # Minimum voltage (p.u.)    
    ...
    model.slack_node_v_sq = pyo.Param(initialize=1.0) # Slack bus voltage (p.u.)
    model.slack_node = pyo.Param()     # Slack bus index.
    
    model.M = pyo.Param(initialize=1e4)    # Big-M constant.
    model.v_penalty_factor = pyo.Param(initialize=1e-3)
    model.i_penalty_factor  = pyo.Param(initialize=1e-3)
