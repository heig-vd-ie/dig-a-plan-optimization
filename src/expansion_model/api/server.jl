...
function handle_request(req::HTTP.Request)
    ...
    try
        if req.method == "POST" && req.target == "/stochastic_planning"
            ...
            println(
                "[$(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))] Running SDDP optimization...",
            )
            ...
            edges = [
                ExpansionModel.Types.Edge(edge_data["id"], edge_data["from"], edge_data["to"]) for edge_data in edges_data
            ]
            ...
            investment_costs = Dict(
                e => rand(investment_cost_range[1]:investment_cost_range[2]) for e in edges
            )
            ...
            println(
                "[$(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))] SDDP optimization completed",
            )

            # Return the response as JSON
            response =
                JSON3.write(Dict("simulations" => simulations, "objectives" => objectives))
            processing_time = (time() - start_time) * 1000
            log_request(req, 200, processing_time)
        ...
        elseif req.method == "GET" && req.target == "/health"
            ...
            processing_time = (time() - start_time) * 1000
            log_request(req, 404, processing_time)
            return HTTP.Response(404, "Not Found")
        end
    catch e
        processing_time = (time() - start_time) * 1000
        println("[$(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))] ERROR: $e")
        log_request(req, 500, processing_time)
        return HTTP.Response(500, "Internal Server Error")
    end
end

println("[$(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))] Starting server on port 8080...")
