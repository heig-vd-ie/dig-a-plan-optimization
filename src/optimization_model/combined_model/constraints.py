...
def combined_model_constraints(model: pyo.AbstractModel) -> pyo.AbstractModel:
    ...
    model.flow_balance = pyo.Constraint(model.SCEN, model.N, rule=imaginary_flow_balance_rule)
    ...
    model.voltage_drop_lower = pyo.Constraint( model.SCEN, model.C, rule=voltage_drop_lower_rule)
    model.voltage_drop_upper = pyo.Constraint( model.SCEN, model.C, rule=voltage_drop_upper_rule)
    model.current_rotated_cone = pyo.Constraint( model.SCEN, model.C, rule=current_rotated_cone_rule)
    ...
    model.current_balance = pyo.Constraint( model.SCEN, model.C, rule=current_balance_rule)
    # 6) Infeasible relaxations (per scenario)
    model.current_limit = pyo.Constraint( model.SCEN, model.C, rule=infeasible_current_limit_rule)
    ...
    model.objective = pyo.Objective(
        rule=ADMM_objective_rule, sense=pyo.minimize)   
    return model
