...
def combined_model_constraints(model: pyo.AbstractModel) -> pyo.AbstractModel:
    ...
    model.node_active_power_balance   = pyo.Constraint(model.N, rule=node_active_power_balance_rule)
...
def node_reactive_power_balance_rule(m, n):
    q_flow_tot = sum(m.q_flow[l, i, j] - m.b[l]/2 * m.v_sq[i] for l, i, j in m.C if i == n)
    if n == m.slack_node:
...
def edge_active_power_balance_rule(m, l):
    ...
    return sum((m.p_flow[l_, i, j] - m.r[l_]/2 * m.i_sq[l_, i, j])
               for l_, i, j in m.C if l_ == l) == 0


def edge_reactive_power_balance_rule(m, l):
    if l in m.S:
        return sum(m.q_flow[l_, i, j] for l_, i, j in m.C if l_ == l) == 0
    return sum((m.q_flow[l_, i, j] - m.x[l_]/2 * m.i_sq[l_, i, j])
               for l_, i, j in m.C if l_ == l) == 0
