...
from optimization_model.slave_model_delta.sets import slave_model_sets
from optimization_model.slave_model_delta.parameters import slave_model_parameters
from optimization_model.slave_model_delta.variables import slave_model_variables
from optimization_model.slave_model_delta.constraints import slave_model_constraints
...
class DigAPlan():
    def __init__(
        ...
        slack_threshold: float = 1e-3, convergence_threshold=1e-4) -> None:
    ...
    def solve_models_pipeline(self, max_iters: int, bender_cut_factor: float = 1e0) -> None:
        ...
        for k in pbar:
            ...
            self.slave_model_instance.master_delta.store_values(master_delta) # type: ignore
            ...
            pbar.set_description(
                f"Master obj: {master_obj:.1E}, Slave obj: {slave_obj:.1E} and Gap: {convergence_result:.1E}"
                )
            if k > 0:
                if abs(convergence_result) < self.convergence_threshold:
                    break
            
            self.add_benders_cuts(nb_iter = k, bender_cut_factor = bender_cut_factor)
        
    
    def add_benders_cuts(self, nb_iter: int, bender_cut_factor = 1e0) -> None:
        constraint_dict = {
            "orientation": 1, 
        }
        # Extract delta results from master model
        delta_value = extract_optimization_results(self.master_model_instance, "delta")
        delta_variable = pl.DataFrame(
        ...
        marginal_cost_df = marginal_cost_df.with_columns(
            ...
            # Split constraint name and index
            c("name").pipe(modify_string_col, format_str= {"]":""}).str.split("[").list.to_struct(fields=["name", "index"])
        ).unnest("name").filter(c("name").is_in(constraint_dict.keys())).rename({"index": "S"}).with_columns(
        ...
        marginal_cost_df = (
            marginal_cost_df
            .filter(c("S").is_in(list(self.slave_model_instance.S))) # type: ignore 
            .join(delta_value, on="S")
            .join(delta_variable, on="S")
            # .with_columns(
            #     # if delta is 0, its marginal cost value should be inverted
            #     (pl.when(c("delta") == 1).then(pl.lit(-1)).otherwise(pl.lit(1))*c("marginal_cost")).alias("marginal_cost")
            # )
        )

        
        ### Store marginal cost values in for further displaying ######################################################
        ...
        for data in marginal_cost_df.to_dicts():
            new_cut += data["marginal_cost"] * (data["delta"] - data["delta_variable"]) 
