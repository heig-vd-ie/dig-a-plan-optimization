...
def compare_dig_a_plan_with_pandapower(dig_a_plan,  net: pp.pandapowerNet):
    ...
    node_voltage_pp = pl.from_pandas(net.res_bus.reset_index())["node_id", "vm_pu", "va_degree"]
    node_voltage = node_voltage.join(node_voltage_pp, on ="node_id", how="left").with_columns((c("v_pu") - c("vm_pu")).abs().alias("v_diff"))

    edge_current = dig_a_plan.extract_edge_current()
    edge_current_pp = pl.concat([
        pl.from_pandas(net.res_line.reset_index())\
            .join(pl.from_pandas(net.line.reset_index())["index", "name"], on="index", how = "left")\
            .select(
                c("name").alias("eq_fk"),
                (1e3*(c("i_from_ka") + c("i_to_ka"))/2).alias("i_pp")
            ),
        pl.from_pandas(net.res_trafo.reset_index())\
            .join(pl.from_pandas(net.trafo.reset_index())["index", "name"], on="index", how = "left")\
            .select(
                c("name").alias("eq_fk"),
                (1e3*c("i_lv_ka")).alias("i_pp")
            )
    ])
