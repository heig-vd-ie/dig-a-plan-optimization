...
def plot_power_flow_results(
    ...
    edge_data: pl.DataFrame = base_grid_data.edge_data.filter(~c("edge_id").is_in(open_switches_id))
    node_data: pl.DataFrame = base_grid_data.node_data
    slack_node_id = base_grid_data.node_data.filter(c("type")== "slack")["node_id"][0]
    nx_tree_grid = generate_tree_graph_from_edge_data(edge_data=edge_data, slack_node_id=slack_node_id)
    ...
    node_data = node_data\
        .join(voltages, on="node_id", how="left")\
        .with_columns(
            ...
            pl.concat_list(c("node_id").cast(pl.Utf8), c("v_pu").round(3).cast(pl.Utf8)).alias("customdata")
        )

    edge_data = edge_data.join(currents["edge_id", "i_pu"], on="edge_id", how="left")\
        .with_columns(
            pl.concat_list(c("u_of_edge", "v_of_edge").replace_strict(coord_mapping, default=None)).alias("coord")
        ).with_columns(
            ...
            pl.concat_list(
                ...
                c("loading").round(1).cast(pl.Utf8)).alias("customdata")
        )
        ...
        if data["type"] == "switch":
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    line=dict(
                        width=edge_width, 
                        color=get_color(edge_colorscale, data["loading"]/max_loading), # From % to pu
                    ),
                    hoverinfo="none",
                    showlegend=False
                )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                ...
                cmin=0, cmax=max_loading,
                colorbar=dict(title="Edge loading [%]", x=1.2) # Title for the color bar
            ),
            showlegend=False,
        )
    )
