...
def plot_grid_from_pandapower(net: pp.pandapowerNet, dig_a_plan, node_size: int = 22, width: int=800,  height: int=700) -> None:
    ...
    switch = switch.with_columns(
        c("closed").replace_strict(switch_mapping,default=None).list.to_struct(fields=["opacity", "color", "dash"]).alias("config"),
    ).unnest("config")
    
    load_id = list(net.load["bus"])
    bus = bus.with_columns(pl.when(c("bus_id").is_in(load_id)).then(pl.lit("circle")).otherwise(pl.lit("square")).alias("symbol"))
    
    fig = go.Figure()

    for data in line.filter(c("max_i_ka") > 1e-3).to_dicts():
    ...
    fig.add_trace(
            go.Scatter(
                ...
                text=switch_id.with_columns("</b>" + c("edge_id").cast(pl.Utf8) + "</b>" )["edge_id"].to_list(),
                mode="markers+text",
                hoverinfo="none",
                showlegend=False,
                textfont=dict(color=switch_id["color"].to_list()),
                marker=dict(
                    size=node_size, color="white", symbol="square"
                ),
            )
        )
