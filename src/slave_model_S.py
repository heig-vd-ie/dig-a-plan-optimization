...
def slave_model_candidate_indexed():
    ...
    In this formulation:
        ...
        - The upstream flow equations become:
                p_z_up(l,i,j) = r[l] * f_c(l,i,j) - p_z_dn(l,i,j)
                q_z_up(l,i,j) = x[l] * f_c(l,i,j) - q_z_dn(l,i,j)
        ...
        - The rotated cone current constraint is written (in squared form) as:
                (2*p_z_up(l,i,j))^2 + (2*q_z_up(l,i,j))^2 + (v_sq[i] - f(l,i,j))^2 <= (v_sq[i] + f(l,i,j))^2.
    ...
    model.F = pyo.Set(model.L, within=model.n * model.n)
    ...
    model.Children = pyo.Set(model.n)
    ...
    model.p = pyo.Param(model.n)         # Real node at bus i.
    model.q = pyo.Param(model.n)         # Reactive load at bus i.
    ...
    model.v_sq = pyo.Var(model.n, bounds=(model.vmin**2, model.vmax**2))
    
    # Candidate-indexed branch variables.
    model.p_z_up = pyo.Var(model.LF, domain=pyo.Reals)
    model.q_z_up = pyo.Var(model.LF, domain=pyo.Reals)
    model.p_z_dn = pyo.Var(model.LF, domain=pyo.Reals)
    model.q_z_dn = pyo.Var(model.LF, domain=pyo.Reals)
    ...
    def node_power_balance_real_rule(m, l, i, j):
        children_sum = sum(m.p_z_up[l2, i2, j2]
                           for (l2, i2, j2) in m.LF if (l2 in m.Children[j] and i2 == j))
        return m.p_z_dn[l, i, j] == m.master_d[l, i, j] * (- m.p[j] - children_sum)
    ...
    def voltage_drop_lower_rule(m, l, i, j):
        expr = m.v_sq[i] - 2*(m.r[l]*m.p_z_up[l, i, j] + m.x[l]*m.q_z_up[l, i, j]) + (m.r[l]**2 + m.x[l]**2)*m.f_c[l, i, j]
    ...
    def voltage_drop_z_upper_rule(m, l, i, j):
        expr = m.v_sq[i] - 2*(m.r[l]*m.p_z_up[l, i, j] + m.x[l]*m.q_z_up[l, i, j]) + (m.r[l]**2 + m.x[l]**2)*m.f_c[l, i, j]
        return m.v_sq[j] - expr <= m.M*(1 - m.master_d[l, i, j]) + m.s[l, i, j]
    model.voltage_drop_z_upper = pyo.Constraint(model.LF, rule=voltage_drop_z_upper_rule)
    ...
    def current_rotated_cone_rule(m, l, i, j):
        lhs = (2*m.p_z_up[l, i, j])**2 + (2*m.q_z_up[l, i, j])**2 + (m.v_sq[i] - m.f[l, i, j])**2
