...
print("Total PVs:", (net.sgen.type == "pv").sum() if "type" in net.sgen.columns else len(net.sgen))
print("Total Nominal Load (MW):", net.load.p_mw.sum())
print("Total Nominal PVs (MW):", net.sgen.loc[net.sgen.type == "pv", "p_mw"].sum() if "type" in net.sgen.columns else net.sgen.p_mw.sum())
...
if idx.size > 0 and t.size >= (idx[0] + 2):
    ...
    print(f"[ADMM] Converged at iteration {k_converged} in {t_converged:.2f} s "
...
print(f"[ADMM] Final residuals: r={r_final:.3e}, s={s_final:.3e}; rho(now)={config.ρ:.6g}")
