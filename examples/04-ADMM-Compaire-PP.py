...
def pp_losses_MW(net: pp.pandapowerNet) -> float:
    ...
    if (
        hasattr(net, "res_trafo")
        and net.res_trafo is not None
...
for N in SCEN_COUNTS:
    ...
    if need_train:
        ...
        z_switch_y = harmonize_by_groups(
            _switch_df_to_dict(zδ_df), groups, rule="majority"
        )
        tap_choice_y = _taps_df_to_choice(zζ_df)

        edge_to_pp_switch = build_edge_to_pp_switch_index_from_dap_and_net(
            dap_train, net
        )
    ...
    if not (
        USE_CACHE and os.path.exists(os.path.join(SAVE_DIR, "normal_open_map.json"))
    ):
        ...
        use_col = (
            ...
            if (
                ...
                and bool(
                    zδ_df_fixed.select(pl.col("normal_open").cast(pl.Int8).sum()).item()
                )
            )
            else "open"
        )
    ...
    grid_test = pandapower_to_dig_a_plan_schema_with_scenarios(
        net,
        number_of_random_scenarios=OOS_N,
        p_bounds=(-0.6, 1.5),
        q_bounds=(-0.1, 0.1),
        v_bounds=(-0.1, 0.1),
        v_min=0.95,
    ...
    for s in range(N):
        ...
        net_s_no = apply_scenario_injections_to_pp_from_nodeedge(
            net_NO, grid_test, scenario=s, s_base_w=S_BASE_W
        )
        try:
            pp.runpp(net_s_no, algorithm="nr")
...
plt.tight_layout()
plt.savefig(
    ".cache/figs/fig_single_box_train_vs_oos500_plus_normalopen.svg",
    bbox_inches="tight",
)
plt.show()
